...
尽管因子在使用的很多时候像一个字符类向量，然而实际上因子是整型向量。一些字符操作函数（比如`gsub()`和`grepl()`）会将因子强制转换成字符，也有一些（比如`nchar()`）会报错，而像`c()`则会用因子内在的整型值。因此，如果需要字符型的操作，最好先将因子强制转换成字符型向量。在R先前的版本中使用因子比字符型向量节省一些内存，现在的版本中确并不是这样。
