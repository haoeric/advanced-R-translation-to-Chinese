...
尽管因子在使用的很多时候像一个字符类向量，然而实际上因子是整型向量。一些字符操作函数（比如`gsub()`和`grepl()`）会将因子强制转换成字符，也有一些（比如`nchar()`）会报错， 而像`c()`则会用因子内在的整型值。

will use the underlying integer values. For this reason, it's usually best to explicitly convert factors to character vectors if you need string-like behaviour. In early versions of R, there was a memory advantage to using factors instead of character vectors, but this is no longer the case. \index{factors|)}
